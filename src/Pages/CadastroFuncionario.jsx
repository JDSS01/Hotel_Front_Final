...
function CadastroFuncionario() {
  ...
  const buscarFuncionario = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/funcionarios/${idParam}`);
  ...
  const salvar = async () => {
    if (!nome || !cpf || !cargo) {
      return mensagemErro('Por favor, preencha o nome, CPF e o cargo.');
    }
    ...
    try {
      if (idParam == null) {
        await axios.post(`${BASE_URL}/funcionarios`, data);
        mensagemSucesso('Funcionário cadastrado com sucesso!');
      } else {
        await axios.put(`${BASE_URL}/funcionarios/${idParam}`, data);
        mensagemSucesso('Funcionário alterado com sucesso!');
      }
      navigate('/funcionarios');
    } catch (error) {
      mensagemErro('Erro ao salvar o funcionário.');
    }
  };
    ...
    <div style={{ backgroundColor: '#2f4146', minHeight: '100vh', padding: '40px', display: 'flex', justifyContent: 'center' }}>
      ...
      <div style={{ backgroundColor: '#f8f9fa', width: '100%', maxWidth: '900px', borderRadius: '15px', padding: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', borderBottom: '10px solid white' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          <h2 style={{ color: '#2c3e50', fontWeight: 'bold' }}>Cadastro de Funcionário</h2>
          <p style={{ color: '#555', fontSize: '14px' }}>Preencha todos os campos para criar o registro do colaborador.</p>
        </div>
        ...
        <div className="row">
          ...
          <div className="col-md-6 mb-3">
            ...
            <select className="form-control" style={{backgroundColor: '#eaeaea'}} value={cargo} onChange={(e) => setCargo(e.target.value)}>
